<template>
  <div>
      ...
      <div class="containerPrincipal">
        <div>
            <table class="tableauEtudiants">
                ...
                <tr v-for="(etudiant, index) in etudiants" :key="etudiant.nom">
                    <td v-if="etudiant.groupe === 'AB'">{{index}}</td>
                    <td v-if="etudiant.groupe === 'AB'">{{etudiant.nom}}</td>
                    <td v-if="etudiant.groupe === 'AB'">{{etudiant.note}}</td>
                </tr>
            </table>
        </div>
        ...
        <h4>Moyenne du groupe AB : {{moyenneGroupe}}</h4>
...
export default {
    ...
    computed:{
        ...
        noteMaxGroupe(){
            ...
            for(let i = 0; i < this.etudiants.length; i++){
                if (this.etudiants[i].note > noteMaxTemp && this.etudiants[i].groupe === 'AB'){
